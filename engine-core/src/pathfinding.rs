    ...
    NF: Fn(Entity) -> Vec<Entity>,
{
    ...
    struct Node { f: u32, space: Entity }
    impl Ord for Node { fn cmp(&self, other: &Self) -> std::cmp::Ordering { other.f.cmp(&self.f) } }
    ...
    let h = |a: Entity, b: Entity| -> u32 { let da = center(a) - center(b); (da.length() as f32) as u32 };

    g_score.insert(start, 0);
    open.push(Node { f: h(start, goal), space: start });
    ...
    let mut neighbors: Vec<Entity> = neighbors_of(space);
        ...
        for nb in neighbors {
            ...
            if tentative < *g_score.get(&nb).unwrap_or(&u32::MAX) {
                ...
                if in_open.insert(nb) { open.push(Node { f, space: nb }); }
            }
        }
    }
    ...
    let mut cur = goal;
    while let Some(&prev) = came_from.get(&cur) { cur = prev; order.push(cur); if cur == start { break; } }
    order.reverse();
    // map to centers
