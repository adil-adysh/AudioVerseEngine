...
fn drain_space_events(mut sink: ResMut<TestEventSink>, mut enter: ResMut<Events<engine_core::events::EnterSpaceEvent>>, mut exit: ResMut<Events<engine_core::events::ExitSpaceEvent>>) {
...
fn navigation_causes_enter_and_exit_space_events() {
    ...
    // Register our test sink and a system to drain events into it each variable update
    ...
    let room = eng.create_room("room-a", glam::vec3(5.0, -1.0, -1.0), glam::vec3(10.0, 1.0, 1.0));
    ...
    assert!(sink.entered.iter().any(|&(_e, s)| s == room), "expected an EnterSpaceEvent for the room");
    assert!(sink.exited.iter().any(|&(_e, s)| s == room), "expected an ExitSpaceEvent for the room");
}
