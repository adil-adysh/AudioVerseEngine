...
fn portal_allows_tagged_entity() {
    ...
    let door = engine_core::components::Shape3D::Aabb(engine_core::components::Aabb {
        min: glam::vec3(4.8, -0.5, -0.5),
        max: glam::vec3(5.2, 0.5, 0.5),
    });
    let _portal = eng.add_portal(
        room_a,
        room_b,
        door,
        true,
    ...
    assert!(
        sink.exited.iter().any(|&(_e, s)| s == room_a),
        "expected an ExitSpaceEvent from room A"
    );
    assert!(
        sink.entered.iter().any(|&(_e, s)| s == room_b),
        "expected an EnterSpaceEvent into room B"
    );
}
...
fn portal_blocks_untagged_entity() {
    ...
    assert!(
        !sink.exited.iter().any(|&(_e, s)| s == room_a),
        "room A exit should be blocked for untagged entity"
    );
}
