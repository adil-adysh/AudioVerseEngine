...
fn can_enter_ocean_only_with_dive() {
    ...
    eng.variable_schedule.add_systems(drain_enter_events);

    // Create ocean at x in [5,10]
    let ocean = eng.create_ocean("ocean", glam::vec3(5.0, -5.0, -5.0), glam::vec3(10.0, 5.0, 5.0));
    ...
    assert!(!sink.entered.iter().any(|&(_e, s)| s == ocean), "should not enter water without CanDive");
    ...
    for _ in 0..80 { eng.update(0.05); }
    let sink = eng.world.resource::<TestEventSink>();
    assert!(sink.entered.iter().any(|&(_e, s)| s == ocean), "expected EnterSpaceEvent after CanDive granted");
}
